...
\[预览拉起一次性完成（强制约定，不得让用户重复下达指令）]

* Date: 2026-09-06

* Context: 用户反馈"为什么最近拉起预览都出现问题"、"不要每次都要多次发指令、浪费我时间"，要求每次拉起预览一次性保证可用

* Category: 行为指令

* Instructions:

  * **每次"拉起预览"必须一步到位、只交一次成果**：一次会话内自动完成后端+前端启动、浏览器实测、确认正常后再向用户报告，禁止让用户在预览失败后再补发"请启动后端""请修复"等后续指令；若失败须当场排查修复到可用，再报告

  * 拉起前一律先确认/启动后端 3001（cd /workspace/backend && node src/app.js）与前端 3000（cd /workspace/frontend && npx vite --host 0.0.0.0）；若已运行则沿用，不重复起多个实例

  * 报告前必须实测数据链路可用：/api/public/grades、/api/public/classes、/api/public/class-trend 等经 vite 3000 代理返回 200 且有真实数据

  * 报告"预览已拉起"必须附带可直接打开的完整预览入口（系统入口 <http://localhost:3000/，或用户指定的预览页），用户在浏览器点开即可用，不需再做任何操作

  * **入口页是每次验证的唯一起点（强制）**：系统入口页 http://localhost:3000/ 必须能正常运行且始终可用，页面包含"移动H5端 / 小程序等效预览 / 管理后台"三个入口卡片。每次拉起预览后，用户验证任何系统效果都必须以该入口页为查看起点，从卡片点入对应端；拉起预览时若入口页任何一处无法渲染/取数失败，必须当场修复后再报告>
...
* Context: 用户发布"1.3.10-等效预览页组件公共化"。以折线图为样板，要求凡"需要被多个页面多次复用"的内容/交互，都必须先提炼成一个公共成型组件，统一渲染标准。核心场景：折线图（看年级为样板）后续还要被看班级、看个人、考试/班级明细等多处复用

* Category: 行为指令

* Instructions:

  * **凡需多处复用为前提的 UI/交互，必须先抽成公共成型组件**，禁止各页面各自内联实现/各自写一套。抽取时以"一处内容为标准"，把所有样式细节（padding、圆角、字号、间距、颜色、图表几何如 padL/padR/padIn、动画、事件交互等）全部提炼进组件本体

  * **原生小程序**：做成小程序语法公共组件（miniprogram/components/\*），页面注册 usingComponents 后引用；**等效预览页**必须做成"等效预览端自己的公共组件"（如 frontend/public/js/\*.js，或多个预览页共用同一份 script），两端各自复用，杜绝等效预览页之间靠"手抄两份"导致漂移

  * **同组件的每处调用，前端样式与交互必须完全一致，只允许后端取数接口不同**：同一组件在不同页面只调用不同接口、渲染不同数据，禁止在同一组件上按页面改样式/交互

  * 组件公共化以同一标准为样板（如折线图以"看年级页为样板"，含 padIn 内侧缩进，保证折线两端不紧贴画布左右缘）：所有几何/交互关键字只在本组件维护一份。若后续发现组件需完善，直接改组件本体，改动即刻同步到所有引用它的位置，提升开发效率、减少重复工作量

  * 原生小程序引小程序组件，等效预览页引自身公共组件，两套组件以"同一套样式/交互标准"为准进行抽取，保证预览端所见即原生端效果

  * **严格复刻样板，严禁改动模板内容**：定下样板后做公共组件，必须原样照抄样板的全部样式与交互细节（含命中判定、悬浮卡片、选中/复位逻辑，如"复位只做静态重绘、绝不重播加载动画"）。严禁在复刻组件的过程中擅自改模板的交互方式或画风——把组件做出来后就当它已验收，复刻时任何"顺手优化/改写"都是违规，会造成开发反复返工

* Date: 2026-09-03

* Context: 用户发布"1.3.10-等效预览页组件公共化优化"。以看年级页折线图为样板，做成公共 trend-line 组件后被看班级复用；用户发现在等效预览页点击数据点，选中效果与悬浮卡片一闪而过、且每次点击都重播加载动画，与原始看年级页模板不一致

* Instructions:

  * **交互复位不允许触发加载动画**：折线图组件里 renderSelection 的复位/松手分支（鼠标移出、touchend/cancel）必须只做静态重绘（drawTrendBase/drawLineTrend 进度1），严禁调 animateLineTrend；加载动效只在首绘/切科目(setItems)时发生。凡抽公共组件必须对照原始样板核对这条行为，防止复刻时把复位误写成重播动画
