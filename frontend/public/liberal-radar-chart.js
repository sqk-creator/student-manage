/* ============================================================
 * LiberalRadarChart 科目均衡分析雷达图组件
 * 与折线图分开封装，共用主色常量，配置相互独立，适配微信小程序
 * 入参：各科原始分数数组 items = [{name, value(原始分), max(满分)}]
 * 组件内部自动换算标准分（标准分 = 原始得分 / 科目满分 x 100）参与绘图与学情判断
 * ============================================================ */
(function (global) {
  var COLORS = {
    primary: '#14A89A',
  ...
  function buildOption(items) {
    ...
    return {
      ...
      radar: {
        ...
        radius: '80%',
        splitNumber: 5,
        name: { textStyle: { color: COLORS.text, fontSize: 11 } },
  ...
  function render(container, items, opts) {
    ...
    opts = opts || {};
    var key = opts.key || container.id || 'radar';
    container.style.background = COLORS.tooltipBg;
    if (!global.echarts) return null;
    var prev = instances[key];
    if (prev && prev.chart) { try { prev.chart.dispose(); } catch (e) {} }
    var chart = global.echarts.init(container, null, { renderer: 'canvas' });
    chart.setOption(buildOption(items));
    bindRadarTooltip(chart, container, items);
    var inst = { chart: chart, container: container, registered: false };
    instances[key] = inst;
    if (!inst.registered) {
      inst.handler = function () {
        var cur = instances[key];
        if (cur && cur.chart) { try { cur.chart.resize(); } catch (e) {} }
      };
  ...
  function bindRadarTooltip(chart, container, items) {
    ...
    function updateSelectName(idx) {
      ...
      for (var i = 0; i < nameEls.length; i++) {
        ...
        var st = el.style || {};
        var want = sel
          ? { fontSize: 14, fontWeight: 700, fill: COLORS.primary, textShadowColor: 'rgba(20,168,154,0.55)', textShadowBlur: 8, textShadowOffsetX: 0, textShadowOffsetY: 0 }
          : { fontSize: 11, fontWeight: 400, fill: COLORS.text, textShadowColor: 'rgba(0,0,0,0)', textShadowBlur: 0, textShadowOffsetX: 0, textShadowOffsetY: 0 };
    ...
    function resetNames() {
      ...
      for (var i = 0; i < nameEls.length; i++) {
        ...
        if (!el) continue;
        var st = el.style || {};
        if (st.fontSize !== 11 || st.fill !== COLORS.text || st.textShadowBlur) {
          el.attr('style', { fontSize: 11, fontWeight: 400, fill: COLORS.text, textShadowColor: 'rgba(0,0,0,0)', textShadowBlur: 0, textShadowOffsetX: 0, textShadowOffsetY: 0 });
          changed = true;
        }
      }
      if (changed) zr.refresh();
    ...
    function nearest(x, y) {
      ...
      var dx = x - cx, dy = y - cy;
      if (Math.sqrt(dx * dx + dy * dy) > r + 24) return -1;
